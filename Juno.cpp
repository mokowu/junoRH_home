...
std::ofstream ofs_dif("/home/grxuser/users/wu/junoRH/diftime.log");
...
void Juno::calcNextTrajectory()
{
  ...
  while(1){
    ...
    if(playflag){ //real time start
      //timeval tv_a,tv_z,tv_dif;
      //gettimeofday(&tv_a,0);
	...
	m_stuck_BIn.read();
      //cout<<"AB "<<m_stuck_A.data<<" "<<m_stuck_B.data<<endl;
      
      ///miso//
      //if(stuck_A>1&&stuck_B>1&&stopflag_A&&stopflag_B){//soket
      //if(m_stuck_A.data<7||m_stuck_B.data<7&&stopflag_A&&stopflag_B){//data port
      // }

      //_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
      if(!start_limit){
	t.tv_nsec += CTRL_PERIOD;
	tsnorm( &t );
	...
	////////////////////////
	//real time over
	if(m_stuck_A.data>num_s&&m_stuck_B.data>num_s){
	  if(!flagNoSleep){
	    t.tv_nsec += CTRL_PERIOD;
	    tsnorm( &t );
	    clock_nanosleep( 0, TIMER_ABSTIME, &t, NULL );
	...
	else if(flagNoSleep){//when suudoori
	  if(m_stuck_A.data<=num_s)
	    m_stuck_A.data+=1;
	  if(m_stuck_B.data<=num_s)
	    m_stuck_B.data+=1;
	}
	else{
	  flagNoSleep=1;
	...
	//////////////
      ...
      //get time
      gettimeofday(&tv_z,0);
      timersub(&tv_z, &tv_a, &tv_dif);
      double dif=tv_dif.tv_usec;
      if(!stopflag_A||!stopflag_B){
   	  tempcount++;
	  temptotal+=dif;
	  difdeque.push_back(dif);
      }
      */
...
void Juno::send_A()
{
  if(flagNoSleep&&m_stuck_A.data>num_s&&m_stuck_B.data<=num_s&&
     m_stuck_B.data<m_stuck_A.data&&stopflag_A&&start_limit){
    //dummy
  }
  else if(!flagNoSleep&&m_stuck_B.data+2<m_stuck_A.data&&stopflag_A&&start_limit){
    //dummy
  }
  else{
    std::lock_guard<std::mutex> lock(mtx_sA);
    server->send2client_A(m_refq_A, relZMP_A, stuck_A);
  }
}

void Juno::send_B()
{
  if(flagNoSleep&&m_stuck_B.data>num_s&&m_stuck_A.data<=num_s&&
     m_stuck_A.data<m_stuck_B.data&&stopflag_B&&start_limit){
    //dummy
  }
  else if(!flagNoSleep&&m_stuck_A.data+2<m_stuck_B.data&&stopflag_B&&start_limit){
    //dummy
  }
  else{
    std::lock_guard<std::mutex> lock(mtx_sB);
    server->send2client_B(m_refq_B, relZMP_B, stuck_B);
  }
}
...
inline void Juno::writeBonly()
{
 //for real time
  clock_gettime( 0, &t );
  /////play/////

  while(!bodyDeque_pB.empty()){
...
{
  /*
  while(!difdeque.empty()){
    //cout<<difdeque.at(0)<<endl;
    ofs_dif<<difdeque.at(0)<<endl;
    difdeque.pop_front();
  }
